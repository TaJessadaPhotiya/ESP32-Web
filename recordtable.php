    ...
    <table class="styled-table" id= "table_id">
      ...
      <tbody id="tbody_table_record">
        <?php
          ...
          //------------------------------------------------------------ กระบวนการในการแสดงตารางบันทึกที่มีข้อมูลเซ็นเซอร์ต่างๆ
          $table = mysqli_query($conn, "SELECT No, Ds, Ult, Ph, Date , Time FROM nodemcu_esp32_table ORDER BY No DESC"); //nodemcu_esp32_table = Youre_table_name
          while($row = mysqli_fetch_array($table))
          {
            echo '<tr>';
            echo '<td class="bdr">'. $row["No"] . '</td>';
            echo '<td class="bdr">'. $row["Ds"] . '</td>';
            echo '<td class="bdr">'. $row["Ult"] . '</td>';
    ...
    <script>
      ...
      function changePage(page) {
        ...
        [...listing_table.getElementsByTagName('tr')].forEach((tr)=>{
            tr.style.display='none'; // รีเซ็ตทั้งหมดเป็นไม่แสดง
        });
        listing_table.rows[0].style.display = ""; // แสดงแถวชื่อเรื่อง
